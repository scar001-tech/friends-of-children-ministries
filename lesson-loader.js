...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (lessonId) {
        const allLessons = JSON.parse(sessionStorage.getItem('cms_lessons') || '[]');
        const found = allLessons.find(l => l.id === parseInt(lessonId));
        if (found) {
            lessonData = {
                title: found.title,
                scripture: found.scripture,
                overview: found.description || "Explore this lesson on " + found.title + ".",
                objectives: [
                    `Understand the core teaching of ${found.title}`,
                    `Explore the scripture text: ${found.scripture}`,
                    `Reflect on how this applies to our daily walk with God`
                ],
                attachments: found.attachments || [],
                groups: {
                    "class-a": {
                        name: "Class A (Ages 3-5)",
                        content: `### Lesson Introduction\nToday we are learning about: **${found.title}**.\n\n### Bible Reading\nLet's read **${found.scripture}**.\n\n### Activity & Play\nAct out the key action in the lesson with high energy. Say a simple prayer together.`,
                        pdfUrl: "#"
                    },
                    "class-b": {
                        name: "Class B (Ages 6-9)",
                        content: `### Lesson Outline\nScripture Focus: **${found.scripture}**.\n\n### Main Story\n${found.description || "No description provided."}\n\n### Class Discussion\n- What stood out to you in this story?\n- How can we practice this in our daily lives?`,
                        pdfUrl: "#"
                    },
                    "class-c": {
                        name: "Class C (10-12)",
                        content: `### Core Study\nLooking closely at **${found.scripture}**.\n\n### Deep Dive\n${found.description || "No description provided."}\n\n### Practical Challenges\nWhat is one concrete thing you will do differently based on this lesson?`,
                        pdfUrl: "#"
                    },
                    "teens": {
                        name: "Teens (Ages 13+)",
                        content: `### Theological Focus\nReading and understanding: **${found.scripture}**.\n\n### Discussion Points\n${found.description || "No description provided."}\n\n### Apologetics & Practice\nHow do we live this out in our schools and social circles?`,
                        pdfUrl: "#"
                    }
                }
            };
    ...
    if (!lessonData) {
        ...
        lessonData = LESSON_CONTENT[lessonKey];

        if (lessonData) {
            // Load custom attachments for seed lessons if saved in cms_lessons
            const allLessons = JSON.parse(sessionStorage.getItem('cms_lessons') || '[]');
            const found = allLessons.find(l => l.id === parseInt(lessonMatch[1]));
            if (found && found.attachments) {
                lessonData.attachments = found.attachments;
            }
        }
    }
    ...
    const renderPlayerOrDownload = (att) => {
        if (att.type === 'audio') {
            return `<div style="width:100%; margin-top:5px;"><audio src="${att.url}" controls style="width: 100%; height: 32px;"></audio></div>`;
        }
        return `
            <a href="${att.url}" class="btn-pdf" download style="align-self: flex-start; padding: 5px 12px; font-size: 0.78rem; height: auto; text-decoration:none; display:inline-flex; align-items:center; justify-content:center;">
                Download File
            </a>
        `;
    };

    // Setup Group Content Switching Function
    const updateGroupContent = (groupId) => {
        const group = lessonData.groups[groupId];
        if (!group || !contentSection) return;

        // Update Meta Group Label
        ...
        // Build dynamic attachments cards
        let attachmentsHtml = '';
        if (lessonData.attachments && lessonData.attachments.length > 0) {
            attachmentsHtml = `
                <div style="margin-top: 30px; border-top: 1px dashed #e2e8f0; padding-top: 25px; text-align: left;">
                    <h4 style="color: var(--text-primary); margin-bottom: 15px; font-size: 1.1rem; font-family: 'Outfit', sans-serif;">Additional Lesson Attachments & Handouts</h4>
                    <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 15px;">
                        ${lessonData.attachments.map(att => `
                            <div class="pdf-card" style="margin: 0; padding: 15px; flex-direction:column; align-items:stretch; gap:10px;">
                                <div class="pdf-info" style="gap: 10px;">
                                    <span class="pdf-icon" style="background: var(--primary-light); padding: 8px; border-radius: 8px; font-size:1.2rem;">
                                        ${getFileEmoji(att.type)}
                                    </span>
                                    <div class="pdf-details">
                                        <h5 style="margin:0; font-size:0.88rem; font-weight:700; color:var(--text-primary); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:140px;" title="${att.name}">${att.name}</h5>
                                        <p style="margin:0; font-size:0.72rem; color:var(--text-muted);">${att.size || '—'}</p>
                                    </div>
                                </div>
                                ${renderPlayerOrDownload(att)}
                            </div>
                        `).join('')}
                    </div>
                </div>
